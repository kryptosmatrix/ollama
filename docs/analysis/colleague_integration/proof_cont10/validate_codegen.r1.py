...
CASES = {'candidate', 'full', 'raw-generator', 'no-messages', 'fixed-content', 'generation', 'go-store', 'gate-self-checks', 'build'}
...
def dependency_manifest():
    ...
    for directory, dirs, files in os.walk(UI/'node_modules', followlinks=False):
        ...
        for name in sorted(files):
            ...
            result[str(p.relative_to(UI))] = ('symlink:' + os.readlink(p)) if p.is_symlink() else digest(p)
    return result
