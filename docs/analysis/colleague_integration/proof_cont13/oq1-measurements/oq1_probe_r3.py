#!/usr/bin/env python3
"""OQ-1 measurement harness, round 3 (continuation 13, Thole): a long conversation whose history fills the context.
...
def run_arm(arm):
    ...
    ask = {'role': 'user', 'content': 'Final question: what is 17 plus 25? Reply with only the number.'}
    msgs_hist = [{'role': 'system', 'content': CARRIER}] + history + [ask]
    probes = [
        ('history_default', {}, msgs_hist, 200),
        ('history_shift_false', {'shift': False}, msgs_hist, 200),
        ('history_keep_all', {'options': {'num_keep': -1}}, msgs_hist, 200),
    ]
    results = []
