#!/usr/bin/env python3
"""OQ-1 measurement harness, round 2 (continuation 13, Thole): keep-whole-prompt lever and reload cost.
...
def run_arm(arm):
    ...
    probes = [
        ('keep_all_long', {'options': {'num_keep': -1}}, msgs_long, 1400),
        ('keep_all_oversize', {'options': {'num_keep': -1}}, msgs_over, 64),
    ]
    results = []
...
if __name__ == '__main__':
    ...
    for arm in ARMS:
        allres += run_arm(arm)
        allres.append({'arm': arm, 'reload_sequence': reload_sequence(arm)})
