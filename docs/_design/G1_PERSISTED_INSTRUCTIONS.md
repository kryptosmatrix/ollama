...
A one-token measurement call is a candidate design, not selected production behaviour. It would make an additional real inference request with the full context and would have to discard all generated text and tool calls without execution, preserve the exact request/model identity, enforce a source-attributable output reserve, account for its own latency/cost, and establish that reported counts cover the untruncated requested input. The small probes do not discharge those obligations. An estimate labelled exact, trusting requested num_ctx as enforced, or renaming a one-token inference a zero-cost tokenizer is rejected. The alternative is an actual model-specific tokenization/admission interface with explicit unavailable results on unqualified backends. The auxiliary one-token-inference option is rejected by the continuation-12 design decision below. The exact non-inferencing admission integration remains OQ-1; it is not silently wired.

### 6.2 Continuation-12 source reconciliation and decision

The pinned native dependency already exposes `POST /v1/chat/completions/input_tokens`: b10091 / commit b4d6c7d8ff69c2e05e4e8ee7e6e710a08abd7b45, `tools/server/server.cpp:255–256`, `server-context.cpp:4703–4705,5295–5343`. It parses the same chat/template/tool input through `oaicompat_chat_params_parse` and counts `tokenize_mixed(...,true,true)` for text; the multimodal branch uses `process_mtmd_prompt(...,true).size()`. These are inspected source contracts, not a successful runtime probe. The read server files match their pinned Git blobs despite three unrelated pre-existing build patches elsewhere in that dependency. Exact numbered excerpts and hashes are retained in continuation-12 native-source/.

The public Ollama `LlamaServer.Tokenize` does not share those special-token flags: `llm/llama_server.go:2463–2465` passes false, whereas the native inference path at `server-context.cpp:4080–4099` passes true,true for non-MTMD input. A flag mismatch is proven; a concrete model undercount is not yet measured. Further, `common/chat.h:249–263` and `common/chat.cpp:2543–2550,2714–2717` show time-dependent template inputs. A preview token count must not be treated as an immutable dispatch receipt across a separate later template render.

Decision: reuse non-inferencing native input preparation/counting, rather than introduce an auxiliary generation call as the production budget mechanism. The final admission decision must inspect the actual prepared input and selected slot, reserve the declared output allowance, and reject before prompt evaluation when it does not fit. The count endpoint can support a point-in-time preview; by itself it neither reserves a slot nor proves a subsequent request used identical template inputs. Extend the existing slot-start admission guard, not a second prompt assembler: `server-context.cpp:3068–3150` reads `slot.task->tokens` and rejects oversized input against `slot.n_ctx` before cache reuse and prompt evaluation. `:1250–1255,1306` caps slot context to the model training context; the Go wrapper’s requested NumCtx is not a substitute. The current guard covers input only, not input plus a reserved output budget. This locates the extension; it does not claim it is implemented.

Remaining OQ-1 decisions: the finite output allowance and its request representation; versioned ingress/capability that an older daemon cannot silently ignore; native-chat/rendered/MLX/media/remote coverage; and exact owned changes/tests before revising the LOC estimate. Unknown or unqualified configurations retain context_unavailable rather than an invented exact count. This does not remove permitted-cloud support from the programme or make a local-only qualification a universal delivery verdict. No source, global safeguard, live model service or real conversation was changed by this decision. Native execution remains unverified: a version-only command timed out, so the model-specific probe was not launched; the follow-up diagnostic was refused before execution.
