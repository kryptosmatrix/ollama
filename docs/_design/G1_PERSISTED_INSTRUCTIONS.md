...
- **A3 — reserve.** The effective context C is the runner's context length after the training-context cap (`llm/server.go:111-115`), equal to the native slot size in every measured run. The default reserve R is C/8 clamped to the range 16–4,096 tokens; a supplied reserve replaces it within A2's bounds. These values are chosen. V1 measured no gap between the daemon's estimate and the runner's own count in 24 cases across three models and both routes (6.5), so the floor of 16 leaves 11 tokens of margin beyond the native clamp of 4 and a single token of slack; a template or model outside that measurement is covered by the margin, not by evidence.
