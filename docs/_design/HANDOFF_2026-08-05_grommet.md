...
**Credentials in a query string were ruled on** (`61449141`), along with the two related questions. They are not refused — refusing needs a rule matching parameter names, and there is no `${env:NAME}` mechanism for URLs to send anyone to — but they are no longer silent either: `Config.Warnings()` says what the configuration costs, at approval, in both surfaces. The false choice was refuse-or-allow; the answer was a channel that could say something else.
